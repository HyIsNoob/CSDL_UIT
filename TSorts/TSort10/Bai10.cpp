...
void Heapify(vector<int>& a, int heapSize, int i, int& dem) {
	int childLeft = i * 2 + 1;int childRight = i * 2 + 2;int max = i;
	if (childLeft < heapSize && a[max] > a[childLeft])//tăng chỉnh a[max]<a[childLeft]
		max = childLeft;
...
void Sort(vector<int>& a) {
	int n = a.size();int heapSize = n;int dem = 0;
